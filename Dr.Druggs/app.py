...
# ----------------------------------------------------------------------------
api_key = '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'
...
def check_conversation_history(user_id):
    ...
    try:
        ...
        # We need to specify an orderBy parameter when using limitToFirst
        entries = db.child("conversations").child(user_id).order_by_child("timestamp").limit_to_first(1).get()
    ...
    try:
        ...
        if entries.each():
            for entry in entries.each():
                ...
                if entry.key() in ["profile", "diary", "myappointments"]:
                    continue
...
def call_openai_with_retry(messages, max_retries=3, delay=0.5):
...
def chat():
    ...
    try:
        ...
        if is_first_time and not user_name and not user_input:
